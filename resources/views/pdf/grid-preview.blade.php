<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center w-full">
            ...
            <a href="{{ route('grid.download-pdf') }}"
               class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold px-5 py-2 rounded-lg shadow-sm transition">
    ...
    <div class="py-10 bg-gray-200 dark:bg-gray-900 min-h-screen">
        ...
        {{-- A4 paper container: ~794px wide, white, elevated shadow --}}
        <div class="mx-auto bg-white shadow-2xl text-gray-800 text-sm" style="width: 794px; min-height: 1123px; padding: 48px;">
            ...
            <table class="w-full border-collapse mb-6" style="table-layout: fixed;">
                ...
                <tbody>
                    @foreach($gridCells->groupBy('row_index') as $rowIndex => $rowCells)
                        <tr>
                            <td class="text-xs text-gray-400 font-bold text-center align-middle" style="width:28px;">R{{ $rowIndex }}</td>
                            @foreach($rowCells->sortBy('column_index') as $cell)
                                @php $fn = $cityFunctions->firstWhere('id', $cell->function_id); @endphp
                                @if($fn)
                                    <td class="border border-gray-200 bg-blue-50 text-center align-middle p-2" style="height:90px;">
                                        @if($fn->image_path)
                                ...
                                @else
                                    <td class="border border-gray-200 bg-gray-50 text-center align-middle" style="height:90px;">
            ...
            <div class="border-t border-gray-200 pt-3 text-center text-gray-400 mt-6 text-xs">
