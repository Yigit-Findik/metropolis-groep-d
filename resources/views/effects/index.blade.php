        ...
        <div class="px-4 sm:px-6 lg:px-8">
            ...
            <div class="overflow-hidden rounded-2xl bg-white hc:bg-black hc:border hc:border-white shadow-sm hc:shadow-none dark:bg-gray-800">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full">
                        <thead class="hidden md:table-header-group border-b border-gray-200 hc:border-white bg-blue-100 hc:bg-neutral-900 dark:border-gray-600 dark:bg-gray-700">
                                <tr>
                        ...
                        <tbody class="divide-y divide-gray-200 hc:divide-white dark:divide-gray-700">
                            @forelse($functions as $function)
                                ...
                                <tr class="md:hidden border-b border-gray-200 hc:border-white dark:border-gray-700">
                                    <td colspan="{{ count($categories) + 1 }}" class="px-4 py-4">
                                        <div>
                                            <div class="flex items-center gap-3 mb-3">
                                                @if($function->image_path)
                                                    <img src="{{ asset($function->image_path) }}" alt="{{ $function->name }}" class="h-8 w-8 object-contain flex-shrink-0">
                                                @endif
                                                <div class="flex items-center gap-2 flex-wrap">
                                                    <span class="font-semibold text-gray-900 hc:text-white dark:text-gray-100">{{ $function->name }}</span>
                                                    @if($function->trashed())
                                                        <span class="inline-flex items-center rounded-full bg-gray-100 hc:bg-neutral-800 hc:border hc:border-white px-2 py-0.5 text-[11px] font-semibold uppercase tracking-wide text-gray-600 hc:text-white dark:bg-gray-700 dark:text-gray-300">Archived</span>
                                                    @endif
                                                </div>
                                            </div>
                                            <div class="space-y-1 divide-y divide-gray-100 hc:divide-white/20 dark:divide-gray-700">
                                                @foreach($categories as $index => $category)
                                                    <div class="flex items-center justify-between gap-4 py-2">
                                                        <span class="text-sm font-medium text-gray-600 hc:text-white dark:text-gray-300 shrink-0">{{ $category }}</span>
                                                        @if($function->trashed())
                                                                    ...
                                                                    <div class="flex flex-col gap-2 items-end">
                                                                        <div class="flex gap-2">
                                                                            <input
                                                                                ...
                                                                                :class="'w-16 rounded border px-2 py-1 text-sm focus:outline-none focus:ring-2 hc:bg-black hc:text-white dark:bg-gray-700 dark:text-white ' + (error ? 'border-red-500 focus:ring-red-500 hc:border-red-400' : 'border-gray-300 dark:border-gray-600 focus:ring-blue-500 hc:border-white hc:focus:ring-yellow-400')">
                                                                            <button
                                                                                @click="save()"
                                                                                :disabled="!isValid()"
                                                                        ...
                                                                        <div class="text-xs text-gray-600 hc:text-white dark:text-gray-400">Range: -10 to +10</div>
                                                                    </div>
                                                                </template>
                                                            </div>
                                                        @endif
                                ...
                                {{-- Desktop row --}}
                                <tr class="hidden md:table-row transition hover:bg-blue-50 hc:hover:bg-neutral-900 dark:hover:bg-gray-700/50">
                                    {{-- Function Name Cell --}}
