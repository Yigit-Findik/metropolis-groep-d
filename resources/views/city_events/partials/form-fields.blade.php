<div class="grid gap-4">
    ...
    {{-- RECURRING SCHEDULE --}}
    <div x-show="{{ $model }}.event_type === 'recurring'"
        ...
        {{-- Step 1: Frequency --}}
        <p class="mb-2 text-xs font-semibold uppercase tracking-wide text-cyan-800 dark:text-cyan-200">Step 1 — Set frequency</p>
        <div class="mb-1 grid gap-4 sm:grid-cols-2">
            <div>
                <label for="{{ $model }}-recurring-frequency-value" class="mb-1 block text-sm font-medium text-slate-700 dark:text-gray-300">
                    How many times <span class="text-red-400">*</span>
                </label>
                <input id="{{ $model }}-recurring-frequency-value" type="number" name="recurring_frequency_value" min="1" max="24"
                       aria-label="How many times the event occurs per unit"
                       x-model.number="{{ $model }}.recurring_frequency_value"
            ...
            <div>
                <label for="{{ $model }}-recurring-frequency-unit" class="mb-1 block text-sm font-medium text-slate-700 dark:text-gray-300">
                    Per <span class="text-red-400">*</span>
                </label>
                <select id="{{ $model }}-recurring-frequency-unit" name="recurring_frequency_unit"
                        aria-label="Select the unit the event repeats in"
    ...
    {{-- ONE-OFF DURATION --}}
    <div x-show="{{ $model }}.event_type === 'one-off'" class="rounded-2xl border border-amber-200 bg-amber-50/80 p-4 dark:border-amber-500/20 dark:bg-amber-500/10">
