<div class="grid gap-4">
    ...
    <div @if($model === 'creating') x-show="creating.event_type === 'recurring'" @else x-show="editing.event_type === 'recurring'" @endif class="rounded-2xl border border-cyan-200 bg-cyan-50/70 p-4 dark:border-cyan-500/20 dark:bg-cyan-500/10">
        ...
        {{-- Active duration --}}
        <p class="mb-2 text-xs font-semibold uppercase tracking-wide text-cyan-800 dark:text-cyan-200">Active window per cycle</p>
        <div class="mb-4 grid gap-4 sm:grid-cols-2">
            <div>
                <label for="{{ $model }}-recurring-active-duration-value" class="mb-1 block text-sm font-medium text-slate-700 dark:text-gray-300">
            ...
            <div>
                <label for="{{ $model }}-recurring-active-duration-unit" class="mb-1 block text-sm font-medium text-slate-700 dark:text-gray-300">
        ...
        {{-- Cycle frequency --}}
        <p class="mb-2 text-xs font-semibold uppercase tracking-wide text-cyan-800 dark:text-cyan-200">Cycle frequency</p>
        <div class="grid gap-4 sm:grid-cols-2">
            <div>
                ...
                <input id="{{ $model }}-recurring-frequency-value" type="number" name="recurring_frequency_value" min="1"
                       ...
                       class="w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm text-slate-900 shadow-sm focus:border-cyan-500 focus:outline-none focus:ring-2 focus:ring-cyan-500/30 disabled:opacity-50 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100">
                <p class="mt-2 text-xs text-slate-500 dark:text-gray-400">Example: 4 times per day = one cycle every 6 hours.</p>
